...
Sometimes, we might have reason to believe that patterns in our data aren't consistent across all values of a variable. One way we can assess this is with conditional means. We'll discuss two ways to get these conditional means -- one where we create data subsets, and one where we alter the `mean()` command itself.

...
```{r}
rm(list = ls())
setwd("/Users/hunte/Desktop/POLS 15 W24/Section7")
load("nba.RData")
```
...
```{r}
model2 <- lm(Wlperc ~ ThreePerc + BLK, data = nba2)
summary(model2)
```
...
```{r}
model3 <- lm(Wlperc ~ ThreePerc + BLK + Attendance, data = nba2)
summary(model3)
```
...
R-squared tells you the proportion of the observed variation in Y that is explained by the model you've specified.
...
So far, we've only used `.csv` data files, but we will also frequently use `.RData` data files in this course. They're a bit easier to use because they were built to cooperate with `R`, so all we need is the `load()` command -- no new name or assignment operator `<-` necessary.
